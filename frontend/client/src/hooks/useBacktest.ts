// ============================================================
// useBacktest Hook - Enhanced with strategy.analyze + strategy.startBacktest flow
// Uses tRPC for all mutations, REST for status polling
// ============================================================
...
export function useBacktest(): UseBacktestReturn {
  ...
  const pollStatus = useCallback(async (rid: string) => {
    try {
      // Use REST API for status polling (works with both in-memory and DB runs)
      const statusData = await api.getRunStatus(rid);

      // Ignore if this isn't the current run
  ...
  /**
   * Main flow: POST /api/runs (python backend) → poll
   */
  ...
  const deploy = useCallback(async (mode: 'paper' | 'live'): Promise<api.DeployResponse> => {
