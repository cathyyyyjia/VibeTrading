// ============================================================
// TradeTable - Trade Execution Log table with CSV export
// Design: Swiss Precision - clean table, mono numbers
// Enhanced: PnL%, Reason columns
// ============================================================
...
export default function TradeTable({ trades, loading, runId }: TradeTableProps) {
  ...
  const handleExportCSV = () => {
    if (!trades) return;
    // Enhanced CSV with pnlPct and reason
    const csv = tradesToCsv(trades as any);
            ...
            {trades.map((trade, index) => (
              <tr
                ...
                className={`hover:bg-muted/20 transition-colors ${
                  index < trades.length - 1 ? 'border-b border-border/40' : ''
                }`}
              >
